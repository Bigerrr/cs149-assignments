...
void saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    cudaDeviceSynchronize(); // 等待GPU运行完毕
    ...
    double copyDuration = kernelStartTime - startTime;
    double kernelDuration = kernelEndTime - kernelStartTime;

    printf("Copy BW from CPU to GPU: %.3f ms\t\t[%.3f GB/s]\n", 1000.f * copyDuration, GBPerSec(2*N*sizeof(float), copyDuration));

    printf("Kernel Process Time: %.3f ms\t\t\t[%.3f GB/s]\n", 1000.f * kernelDuration, GBPerSec(totalBytes, kernelDuration));
    printf("-----------------------------------------------------------\n");
    //
